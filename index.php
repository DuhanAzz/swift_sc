...
<body class="bg-ocean text-slate-100 antialiased relative overflow-x-hidden min-h-screen">
    ...
    <header class="relative py-24 px-4 text-center overflow-hidden">
        <?php if (file_exists('admin/uploads/hero_bg.jpg')): ?>
            <div class="absolute inset-0 bg-cover bg-center bg-no-repeat -z-20" style="background-image: url('admin/uploads/hero_bg.jpg?v=<?= time() ?>');"></div>
            <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-[2px] -z-10 mix-blend-multiply"></div>
        <?php else: ?>
            <div class="absolute inset-0 bg-gradient-to-b from-teal-900/10 to-transparent -z-10"></div>
        <?php endif; ?>
        
        <h2 class="font-cursive text-5xl md:text-7xl mb-2 text-white drop-shadow-lg tracking-wide transform -rotate-2"><?= htmlspecialchars($cmsData['hero_title']) ?></h2>
        ...
        <p class="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto font-medium mb-10 drop-shadow"><?= htmlspecialchars($cmsData['hero_desc']) ?></p>
