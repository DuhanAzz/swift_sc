...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($jenis_kelamin) || empty($tanggal_lahir) || empty($id_kolam) || empty($sekolah)) {
        echo json_encode(["status" => "gagal", "message" => "Mohon lengkapi semua data wajib (termasuk Asal Sekolah)."]);
        exit;
    }
    ...
    try {
        ...
        $stmt = $koneksi->prepare("INSERT INTO calon_member (nama, jenis_kelamin, no_hp, sekolah, tanggal_lahir, cabang_id, tanggal_daftar, payment_status, status_approval) VALUES (?, ?, ?, ?, ?, ?, ?, 'Unpaid', 'Pending')");
        
        if ($stmt) {
            $stmt->bind_param("sssssis", $nama, $jenis_kelamin, $no_hp, $sekolah, $tanggal_lahir, $id_kolam, $tgl_gabung);
            if ($stmt->execute()) {
