    ...
    <div class="p-4 rounded-lg mt-14">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-4">
                ...
                $coaches = [];
                try {
                    $docs = $database->getDocuments('coaches');
                    foreach ($docs as $d) {
                        if (($d['id_kolam'] ?? '') == $admin_pool_id) {
                            $coaches[] = $d;
                        }
                    }
                } catch (\Exception $e) {}
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-4 hover:shadow-md transition-shadow">
                    ...
                    <div>
                        <h3 class="font-bold text-gray-800"><?= htmlspecialchars($c['nama_pelatih']) ?></h3>
                        <p class="text-xs text-blue-600 font-semibold"><?= htmlspecialchars($c['jabatan'] ?? 'Pelatih') ?></p>
                        <p class="text-xs text-gray-400 mt-1"><i class="fa fa-phone"></i> <?= htmlspecialchars($c['no_hp']) ?></p>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto p-4">
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody>
                                ...
                                $performances = [];
                                try {
                                    $pDocs = $database->getDocuments('performances');
                                    foreach ($pDocs as $p) {
                                        if (($p['pool_id'] ?? '') == $admin_pool_id) {
                                            $performances[] = $p;
                                        }
                                    }
                                    // Urutkan DESC
                                ...
                                if(count($performances) > 0) {
                                    foreach($performances as $p) {
                                        ...
                                        if(isset($p['member_id'])) {
                                            try {
                                                $atletDoc = $database->getDocument('atlet', $p['member_id']);
                                                if($atletDoc) $nama_atlet = $atletDoc['nama'] ?? 'Unknown';
                                            } catch (\Exception $e) {}
                                        }
                                ?>
                                <tr class="border-b hover:bg-orange-50 transition-colors">
                                    <td class="px-4 py-3 text-gray-400"><?= isset($p['record_date']) ? date('d M Y', strtotime($p['record_date'])) : '-'; ?></td>
                                    <td class="px-4 py-3 font-bold text-gray-800"><?= htmlspecialchars($nama_atlet); ?></td>
                                    <td class="px-4 py-3">
                                        <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-0.5 rounded"><?= htmlspecialchars($p['swim_style'] ?? '-'); ?></span>
                                        <span class="text-xs text-gray-500 ml-1"><?= htmlspecialchars($p['distance'] ?? '-'); ?>m</span>
                                    </td>
                                    <td class="px-4 py-3 text-center font-bold text-slate-700 bg-slate-50"><?= htmlspecialchars($p['time_formatted'] ?? '-'); ?></td>
                                    <td class="px-4 py-3 text-right text-xs text-gray-500"><?= htmlspecialchars($p['recorded_by_name'] ?? 'Pelatih'); ?></td>
                                </tr>
