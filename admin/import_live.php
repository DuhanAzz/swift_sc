...
if(isset($_POST['hapus_semua'])) {
    $q = mysqli_query($koneksi, "DELETE FROM member WHERE cabang_id = '$admin_pool_id'");
    if($q) {
        echo "<script>alert('Semua data atlet di cabang ini berhasil dihapus!'); window.location='atlet.php';</script>";
    } else {
...
if(isset($_POST['import'])) {
    if($_FILES['file_csv']['error'] == 0) {
        ...
        $first_line_str = strtolower(implode(',', $first_line));
        
        if (strpos($first_line_str, 'timestamp') !== false || strpos($first_line_str, 'nama lengkap orang tua') !== false) {
            $format = 'google_form';
        } else if (strpos($first_line_str, 'data atlet') !== false || strpos($first_line_str, 'no.') !== false || empty(trim($first_line[0]))) {
            $format = 'excel_split';
        }
        ...
        if ($format == 'google_form') {
            ...
            while (($row = fgetcsv($file)) !== FALSE) {
                ...
                if(empty(trim($nama_siswa))) continue;
                ...
                $tanggal_lahir_db = (count($tl_parts) == 3) ? $tl_parts[2] . '-' . $tl_parts[1] . '-' . $tl_parts[0] : date('Y-m-d');

                $tanggal_mendaftar = $row[11] ?? '';
                $tm_parts = explode('/', $tanggal_mendaftar);
                $tanggal_gabung_db = (count($tm_parts) == 3) ? $tm_parts[2] . '-' . $tm_parts[1] . '-' . $tm_parts[0] : date('Y-m-d');

                insert_atlet($koneksi, $nama_siswa, 'L', $no_hp, $tanggal_lahir_db, $admin_pool_id, $tanggal_gabung_db, 'Pemula', $sekolah, $success, $fail);
            }
        } 
            ...
            while (($row = fgetcsv($file)) !== FALSE) {
                ...
                if (empty(trim($row[0])) && empty(trim($row[1]))) {
                    if (strpos(strtoupper($full_row), 'MENENGAH') !== false) $current_kelas = 'Menengah';
                    elseif (strpos(strtoupper($full_row), 'PEMULA') !== false) $current_kelas = 'Pemula';
                    continue;
                }
                if (strtolower(trim($row[0])) == 'no.' || empty(trim($row[1]))) continue;

                $nama_siswa = bersihkan_input($row[1]);
                $tanggal_lahir_raw = $row[4] ?? '';
                $sekolah = bersihkan_input($row[6] ?? '');
                
                if (!empty(trim($tanggal_lahir_raw))) {
                    ...
                    } else $tanggal_lahir_db = date('Y-m-d');
                } else $tanggal_lahir_db = date('Y-m-d'); 

                insert_atlet($koneksi, $nama_siswa, 'L', '-', $tanggal_lahir_db, $admin_pool_id, date('Y-m-d'), $current_kelas, $sekolah, $success, $fail);
            }
        } else {
            echo "<script>alert('Format CSV tidak dikenali!'); window.location='atlet.php';</script>";
            exit;
        }
...
function insert_atlet($koneksi, $nama, $jk, $hp, $tgl_lahir, $cabang_id, $tgl_gabung, $kelas, $sekolah, &$success, &$fail) {
    ...
    $next_id = 1;
    if($q_last && $r = mysqli_fetch_assoc($q_last)) $next_id = $r['id'] + 1;
    ...
    if (mysqli_query($koneksi, $query)) $success++; else $fail++;
}
?>
<!DOCTYPE html>
<html>
...
<body class="bg-gray-50 min-h-screen flex items-center justify-center p-6" style="font-family: 'Plus Jakarta Sans', sans-serif;">
    <div class="max-w-xl w-full bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        <div class="mb-8 text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-900">Import Data Atlet Otomatis</h2>
            <p class="text-gray-500 text-sm mt-2">Sistem sekarang akan <b>otomatis mendeteksi format</b> CSV Anda (baik dari Google Form maupun file Excel Umbang Tirta).</p>
        </div>
        
        <form method="POST" enctype="multipart/form-data" class="space-y-6 mb-6">
            <div>
        ...
        <div class="bg-red-50 p-4 rounded-xl border border-red-100">
            ...
            <p class="text-red-600 text-xs mb-4">Jika data Anda berantakan akibat salah format sebelumnya, Anda dapat mereset (menghapus) seluruh data atlet di cabang ini dengan tombol di bawah.</p>
            <form method="POST" onsubmit="return confirm('PERINGATAN KERAS! Apakah Anda yakin ingin MENGHAPUS SEMUA DATA ATLET di cabang ini? Tindakan ini tidak dapat dibatalkan!');">
                <button type="submit" name="hapus_semua" class="w-full bg-red-600 text-white font-bold py-2 px-4 rounded-xl hover:bg-red-700 transition-colors">
                    Hapus Semua Data di Cabang Ini
                </button>
            </form>
        </div>
    </div>
